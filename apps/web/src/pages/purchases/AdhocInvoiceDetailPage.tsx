...
function numberToWords(num: number): string {
    ...
    let result = convert(rupees) + ' Rupees'; if (paise > 0) result += ' and ' + convert(paise) + ' Paise'; return result + ' Only';
}
...
export default function AdhocInvoiceDetailPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            } catch { message.error('Failed to load Adhoc Invoice'); } finally { setLoading(false); }
        };
        if (id) fetchData();
    }, [id]);
    ...
    const enrichedItems = useMemo(() => {
        ...
        return po.items.map((item, idx) => {
            const inv = getInvItem(item.item_id); const taxableAmount = item.ordered_quantity * item.unit_price;
    ...
    const handleMarkPaid = async () => { if (!po) return; try { const updated = await purchasesApi.update(po.id, { invoice_status: 'complete' }); message.success('Adhoc Invoice marked as paid'); setPo(updated); } catch { message.error('Failed to update'); } };
    const isPaid = po?.invoice_status === 'complete';
    const handleStartEdit = () => { if (!po || isPaid) return; setEditItems(po.items.map((item, idx) => ({ key: idx, item_id: item.item_id, ordered_quantity: item.ordered_quantity, unit_price: item.unit_price }))); setEditNotes(po.notes || ''); setEditDeliveryDate(po.expected_delivery_date ? dayjs(po.expected_delivery_date) : null); setIsEditing(true); };
    ...
    const handleSaveEdit = async () => {
        ...
        if (validItems.length === 0) { message.error('Add at least one valid item'); return; }
        setSaving(true);
        try {
            ...
            setPo(updated); setIsEditing(false); message.success('Adhoc Invoice updated');
    ...
    const handleCopyToBuyer = () => {
        ...
        const sel = allBuyers.find(b => b.id === selectedBuyerId); if (!sel) return;
        ...
        sessionStorage.setItem('adoc_copy_items', JSON.stringify(copiedItems)); sessionStorage.setItem('adoc_copy_source', po.po_number);
        setBuyerSelectModalOpen(false); navigate(`/app/purchases/adhoc-invoice/create?buyerId=${selectedBuyerId}&buyerName=${encodeURIComponent(sel.name)}`);
    };
        ...
        <div style={{ padding: '0 0 32px', maxWidth: '100%', overflowX: 'hidden' }} className="print-area">
            ...
            <div className="bg-white rounded-lg border border-gray-200 no-print" style={{ padding: '12px 24px', marginBottom: 20 }}>
                <div className="flex justify-between items-center">
                    <Space size="small"><Text strong style={{ fontSize: 14 }}>ADHOC INVOICE DETAILS</Text><Tag color="orange" style={{ borderRadius: 12 }}>Adhoc</Tag></Space>
                    {isEditing ? (
                        ...
                        <Space size="small">
                            ...
                            <Tooltip title={isPaid ? 'Cannot edit paid invoice' : 'Edit'}><Button icon={<EditOutlined />} onClick={handleStartEdit} disabled={isPaid} style={isPaid ? {} : { borderColor: '#1677ff', color: '#1677ff' }}>Amend</Button></Tooltip>
            ...
            <div className="bg-white rounded-lg border border-gray-200" style={{ padding: '32px 40px' }}>
                <Title level={3} style={{ textAlign: 'center', marginBottom: 28 }}>Adhoc Invoice</Title>
                ...
                <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', marginBottom: 28 }}>
                    <div style={{ textAlign: 'center', background: '#f3f4f6', padding: 10, borderBottom: '1px solid #e5e7eb' }}><Text strong style={{ fontSize: 13 }}>Adhoc Invoice Details</Text></div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr' }}>
                        <div style={{ padding: '12px 16px', borderRight: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}><Text style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>Invoice Number</Text><Text strong style={{ fontSize: 13 }}>{po.po_number}</Text></div>
                        <div style={{ padding: '12px 16px', borderRight: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}><Text style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>Invoice Date</Text><Text strong style={{ fontSize: 13 }}>{dayjs(po.order_date).format('DD/MM/YYYY')}</Text></div>
                        <div style={{ padding: '12px 16px', borderRight: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}><Text style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>Due Date</Text>{isEditing ? <DatePicker size="small" value={editDeliveryDate} onChange={setEditDeliveryDate} format="DD/MM/YYYY" style={{ width: '100%' }} /> : <Text strong style={{ fontSize: 13 }}>{po.expected_delivery_date ? dayjs(po.expected_delivery_date).format('DD/MM/YYYY') : '-'}</Text>}</div>
                        <div style={{ padding: '12px 16px', borderBottom: '1px solid #e5e7eb' }}><Text style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>Payment Status</Text><Tag color={isPaid ? 'green' : 'orange'} style={{ borderRadius: 12 }}>{isPaid ? 'Paid' : 'Unpaid'}</Tag></div>
                        ...
                        <div style={{ padding: '12px 16px' }}><Text style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>Invoice Amount</Text><Text strong style={{ fontSize: 13, color: '#0f766e' }}>{isEditing ? formatCurrency(editTotals.grandTotal) : formatCurrency(po.total_amount || totals.grandTotal)}</Text></div>
                    </div>
                </div>
                {!isEditing ? (<>
            ...
            {!isEditing && (<div className="flex justify-between items-center mt-6 no-print">
                <Button icon={<CheckCircleOutlined />} style={{ borderColor: isPaid ? '#d9d9d9' : '#52c41a', color: isPaid ? '#bfbfbf' : '#52c41a', fontWeight: 600, height: 40 }} size="large" onClick={handleMarkPaid} disabled={isPaid}>{isPaid ? 'Paid' : 'Mark As Paid'}</Button>
                <Button type="default" icon={<DollarOutlined />} size="large" style={{ borderColor: '#722ed1', color: '#722ed1', fontWeight: 600, height: 40 }}>Record Payment</Button>
            </div>)}
            ...
            <Modal title="Select Buyer" open={buyerSelectModalOpen} onCancel={() => setBuyerSelectModalOpen(false)} footer={[<Button key="c" onClick={() => setBuyerSelectModalOpen(false)}>Cancel</Button>, <Button key="ok" type="primary" onClick={handleCopyToBuyer} disabled={!selectedBuyerId}>Continue</Button>]} width={500} centered><div style={{ padding: '16px 0' }}><div style={{ marginBottom: 16 }}>Select a buyer:</div><div style={{ display: 'flex', gap: 8 }}><Select style={{ flex: 1 }} showSearch placeholder="Search buyer" loading={loadingBuyers} value={selectedBuyerId} onChange={setSelectedBuyerId} options={allBuyers.map(b => ({ value: b.id, label: b.name }))} optionFilterProp="label" /><Button type="primary" icon={<PlusOutlined />} onClick={() => setAddCompanyOpen(true)}>Add Company</Button></div></div></Modal>
