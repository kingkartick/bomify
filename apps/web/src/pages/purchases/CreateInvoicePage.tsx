...
import SupplierDetailsDrawer, { SupplierDetails } from '@/features/purchases/components/drawers/SupplierDetailsDrawer';
...
export default function CreateInvoicePage() {
    ...
    const supplierNameParam = searchParams.get('buyerName');
    ...
    const [supplier, setSupplier] = useSessionStorage<Party | null>('inv_draft_supplier', null);
    ...
    const [placeOfSupplySource, setPlaceOfSupplySource] = useSessionStorage<'company' | 'supplier' | 'delivery'>('inv_pos_source', 'delivery');
    
    const [supplierDetails, setSupplierDetails] = useLocalStorage<SupplierDetails>('inv_supplier_details', {});
    const [userAddress, setUserAddress] = useLocalStorage<any>('inv_user_address', { address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India' });
    const [deliveryLocation, setDeliveryLocation] = useLocalStorage<any>('inv_delivery_location', { locationName: 'Main', address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India', billingLocations: [] });
    ...
    useEffect(() => {
        if (currentUser && !supplierDetails.companyName) {
            setSupplierDetails({ companyName: currentUser.full_name || currentUser.username || "Checking", gstin: currentUser.id ? `27AADCB2230M1Z${currentUser.id}` : '', address: currentUser.email || "Main Address,", city: "Mumbai", state: "Maharashtra", pincode: "400001" });
        }
    }, [currentUser, supplierDetails.companyName]);

    // Handle copied PO items
    useEffect(() => {
        const storedItems = sessionStorage.getItem('inv_copy_items');
        const sourcePo = sessionStorage.getItem('inv_copy_source');
        if (storedItems) {
            try {
                ...
                sessionStorage.removeItem('inv_copy_items');
                sessionStorage.removeItem('inv_copy_source');
    ...
    const handleSave = async (status: 'draft' | 'sent') => {
        try {
            ...
            const orderNum = values.invoice_number || `INV-${Date.now()}`;
            ...
            // Clear storage
            ['inv_draft_supplier', 'inv_pos_source'].forEach(k => sessionStorage.removeItem(k));
            ['inv_supplier_details', 'inv_user_address', 'inv_delivery_location'].forEach(k => localStorage.removeItem(k));
            message.success(status === 'draft' ? 'Invoice saved as draft' : 'Invoice created successfully!');
            if (status === 'sent') {
                navigate(`/app/purchases/invoice/${result.id}?buyerName=${encodeURIComponent(supplier?.name || supplierNameParam || '')}`);
            } else {
    ...
    return (
        <div style={{ maxWidth: '1536px', width: '100%', margin: '0 auto', paddingBottom: '40px' }}>
                    ...
                    <Col span={8}>
                        <div className="flex flex-col gap-4 h-full">
                            <Card size="small" className="border border-gray-200 shadow-sm rounded-xl overflow-hidden bg-white" title={<div className="text-slate-700 font-semibold text-[13px] uppercase tracking-wide">Supplier Details</div>} extra={<Button type="text" size="small" icon={<EditOutlined className="text-slate-400 hover:text-blue-600" />} onClick={() => setIsSupplierDrawerOpen(true)} />} styles={{ header: { backgroundColor: '#fcfcfd', borderBottom: '1px solid #f1f5f9', padding: '12px 16px', minHeight: 'auto' }, body: { padding: '16px' } }}>
                                <div className="mb-4"><Text strong className="text-[15px] text-slate-800">{supplierDetails.companyName}</Text><div className="text-slate-500 text-[12px] mt-1">GSTIN: <span className="text-slate-700 font-medium">{supplierDetails.gstin}</span></div></div>
            ...
            {/* Drawers & Modals */}
            <SupplierDetailsDrawer open={isSupplierDrawerOpen} onClose={() => setIsSupplierDrawerOpen(false)} supplier={supplierDetails} onSave={setSupplierDetails} />
            <SupplierDetailsDrawer open={isSupplierDrawerOpen} onClose={() => setIsSupplierDrawerOpen(false)} supplier={(supplier as unknown as SupplierDetails) || {}} onSave={(data) => setSupplier(data as Party)} />
