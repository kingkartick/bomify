...
export default function CreateServiceConfirmationPage() {
    ...
    const [buyer, setBuyer] = useSessionStorage<Party | null>('sc_draft_buyer', null);
    ...
    const [placeOfSupplySource, setPlaceOfSupplySource] = useSessionStorage<'company' | 'buyer' | 'delivery'>('sc_pos_source', 'delivery');
    
    const [buyerDetails, setBuyerDetails] = useLocalStorage<BuyerDetails>('sc_buyer_details', {});
    const [userAddress, setUserAddress] = useLocalStorage<any>('sc_user_address', { address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India' });
    const [deliveryLocation, setDeliveryLocation] = useLocalStorage<any>('sc_delivery_location', { locationName: 'Main', address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India', billingLocations: [] });
    ...
    useEffect(() => {
        const storedItems = sessionStorage.getItem('sc_copy_items');
        const sourcePo = sessionStorage.getItem('sc_copy_source');
        if (storedItems) {
            try {
                ...
                sessionStorage.removeItem('sc_copy_items');
                sessionStorage.removeItem('sc_copy_source');
    ...
    const handleSave = async (status: 'draft' | 'sent') => {
        try {
            ...
            const scNumber = values.so_number || `SC-${Date.now()}`;
            ...
            // Clear storage
            ['sc_draft_buyer', 'sc_pos_source'].forEach(k => sessionStorage.removeItem(k));
            ['sc_buyer_details', 'sc_user_address', 'sc_delivery_location'].forEach(k => localStorage.removeItem(k));
            message.success(status === 'draft' ? 'Service Confirmation saved as draft' : 'Service Confirmation created successfully!');
            if (status === 'sent') {
                navigate(`/app/purchases/service-confirmation/${result.id}?buyerName=${encodeURIComponent(buyer?.name || buyerNameParam || '')}`);
            } else {
        ...
        <div style={{ maxWidth: '1536px', width: '100%', margin: '0 auto', paddingBottom: '40px' }}>
            ...
            <div className="bg-[#001529] text-white p-3 px-6 rounded-t-lg flex justify-between items-center mt-2 shadow-sm">
                <Space size="middle">
                    ...
                    <Title level={4} style={{ margin: 0, color: 'white' }}>Service Confirmation</Title>
                </Space>
                ...
                <Row gutter={24}>
                    ...
                    <Col span={8}>
                        <Card size="small" className="border border-blue-100 shadow-[0_4px_12px_rgba(37,99,235,0.06)] rounded-xl bg-gradient-to-b from-white to-[#f8fafc] h-full overflow-hidden" title={<div className="text-blue-700 font-semibold text-[13px] uppercase tracking-wide">Primary Document Details</div>} extra={<Button type="text" size="small" className="text-xs text-slate-500 hover:text-slate-800 bg-white border border-slate-200 shadow-sm rounded-md px-2" onClick={() => setShowOptionalFields(!showOptionalFields)}>{showOptionalFields ? 'Hide Optional Fields' : 'Show Optional Fields'}</Button>} styles={{ header: { backgroundColor: '#f0f4f8', borderBottom: '1px solid #e2e8f0', padding: '12px 16px', minHeight: 'auto' }, body: { padding: '16px 20px' } }}>
                            <Form.Item label={<span className="text-xs font-medium text-slate-500 uppercase tracking-wide">Title <span className="text-red-500">*</span></span>} style={{ marginBottom: 20 }}><Input defaultValue={`Service Confirmation dated ${dayjs().format('DD/MM/YYYY')}`} className="rounded-lg border-gray-300 py-1.5 font-medium text-slate-800" /></Form.Item>
