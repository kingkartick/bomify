...
export default function CreateAdhocInvoicePage() {
    ...
    const [buyer, setBuyer] = useSessionStorage<Party | null>('adoc_draft_buyer', null);
    ...
    const [placeOfSupplySource, setPlaceOfSupplySource] = useSessionStorage<'company' | 'buyer' | 'delivery'>('adoc_pos_source', 'delivery');
    
    const [buyerDetails, setBuyerDetails] = useLocalStorage<BuyerDetails>('adoc_buyer_details', {});
    const [userAddress, setUserAddress] = useLocalStorage<any>('adoc_user_address', { address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India' });
    const [deliveryLocation, setDeliveryLocation] = useLocalStorage<any>('adoc_delivery_location', { locationName: 'Main', address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India', billingLocations: [] });
    ...
    const handleSave = async (status: 'draft' | 'sent') => {
        try {
            ...
            const adocNumber = values.adoc_number || `ADOC-${Date.now()}`;
            const result = await purchasesApi.create({
                po_number: adocNumber,
            ...
            ['adoc_draft_buyer', 'adoc_pos_source'].forEach(k => sessionStorage.removeItem(k));
            ['adoc_buyer_details', 'adoc_user_address', 'adoc_delivery_location'].forEach(k => localStorage.removeItem(k));
            message.success(status === 'draft' ? 'Adhoc Invoice saved as draft' : 'Adhoc Invoice created successfully!');
            if (status === 'sent') {
                navigate(`/app/purchases/adhoc-invoice/${result.id}?buyerName=${encodeURIComponent(buyer?.name || buyerNameParam || '')}`);
            } else {
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.writeFile(wb, 'adhoc_invoice_items.xlsx');
    };
    ...
    const baseColumns = [
        { title: 'Item ID', dataIndex: 'itemId', key: 'itemId', width: 200, render: (_: any, record: POLineItem) => (
            <Select style={{ width: 180 }} placeholder="Select or type" value={record.itemId} onChange={(val) => handleItemSelect(record.key, val)} options={itemDropdownOptions} showSearch optionFilterProp="label" allowClear onClear={() => handleItemSelect(record.key, null)} />
        )},
        { title: 'Item Description', dataIndex: 'description', key: 'description', render: (_: any, record: POLineItem) => <Input value={record.description} onChange={e => handleFieldChange(record.key, 'description', e.target.value)} placeholder="Enter description (adhoc)" /> },
        ...
        <div style={{ maxWidth: '1536px', width: '100%', margin: '0 auto', paddingBottom: '40px' }}>
            ...
            <div className="bg-[#001529] text-white p-3 px-6 rounded-t-lg flex justify-between items-center mt-2 shadow-sm">
                <Space size="middle">
                    ...
                    <Title level={4} style={{ margin: 0, color: 'white' }}>Adhoc Invoice</Title>
                </Space>
                <div className="flex items-center gap-4">
                    ...
                    <Button onClick={() => navigate('/app/purchases')} icon={<CloseOutlined />} danger ghost style={{ borderColor: '#ff4d4f', color: '#ff7875', borderRadius: 6, fontWeight: 500 }}>Cancel</Button>
                </div>
            </div>
                    ...
                    <Col span={8}>
                        <Card size="small" className="border border-blue-100 shadow-[0_4px_12px_rgba(37,99,235,0.06)] rounded-xl bg-gradient-to-b from-white to-[#f8fafc] h-full overflow-hidden" title={<div className="text-blue-700 font-semibold text-[13px] uppercase tracking-wide">Primary Document Details</div>} extra={<Button type="text" size="small" className="text-xs text-slate-500 hover:text-slate-800 bg-white border border-slate-200 shadow-sm rounded-md px-2" onClick={() => setShowOptionalFields(!showOptionalFields)}>{showOptionalFields ? 'Hide Optional Fields' : 'Show Optional Fields'}</Button>} styles={{ header: { backgroundColor: '#f0f4f8', borderBottom: '1px solid #e2e8f0', padding: '12px 16px', minHeight: 'auto' }, body: { padding: '16px 20px' } }}>
                            ...
                            {showOptionalFields && (<>
                                ...
                                <Row gutter={20}>
                                    <Col span={12}><Form.Item label={<span className="text-[11px] font-medium text-slate-400 uppercase tracking-wide">PO Reference</span>} style={{ marginBottom: 20 }}><Input className="rounded-lg border-gray-200 bg-slate-50 hover:bg-white hover:border-blue-400 focus:border-blue-500 py-1.5" /></Form.Item></Col>
                                    <Col span={12}><Form.Item label={<span className="text-[11px] font-medium text-slate-400 uppercase tracking-wide">PO Date</span>} style={{ marginBottom: 20 }}><DatePicker style={{ width: '100%' }} className="rounded-lg border-gray-200 bg-slate-50 hover:bg-white hover:border-blue-400 focus:border-blue-500 py-1.5" format="DD/MM/YYYY" /></Form.Item></Col>
                                </Row>
                                <Row gutter={20}>
                                ...
                                <Form.Item label={<span className="text-[11px] font-medium text-slate-400 uppercase tracking-wide">Remarks</span>} style={{ marginBottom: 0 }}><Input.TextArea rows={2} placeholder="Enter remarks..." className="rounded-lg border-gray-200 bg-slate-50 hover:bg-white hover:border-blue-400 focus:border-blue-500" /></Form.Item>
                            </>)}
                        </Card>
                    </Col>
                </Row>
                        ...
                        <Col span={10}>
                            <div className="p-4 rounded-md">
                                ...
                                <div className="flex justify-between mb-3 items-center">
                                    <Space size="middle">
                                        <Button size="small" onClick={() => setRcmEnabled(!rcmEnabled)} style={{ borderRadius: 4, fontWeight: 700, fontSize: 13, padding: '2px 12px', border: rcmEnabled ? '2px solid #1677ff' : '2px solid #91caff', background: rcmEnabled ? '#e6f4ff' : '#fff', color: rcmEnabled ? '#1677ff' : '#4096ff', boxShadow: rcmEnabled ? '0 0 0 2px rgba(22,119,255,0.15)' : 'none' }}>RCM{rcmEnabled ? ' ✓' : ''}</Button>
                                ...
                                <div className="flex justify-between mb-4 items-center">
                                    <Space size="middle">
                                        <Button size="small" onClick={() => setRoundOffEnabled(!roundOffEnabled)} style={{ borderRadius: 4, fontWeight: 700, fontSize: 13, padding: '2px 12px', border: roundOffEnabled ? '2px solid #52c41a' : '2px solid #b7b7b7', background: roundOffEnabled ? '#f6ffed' : '#fff', color: roundOffEnabled ? '#52c41a' : '#595959', boxShadow: roundOffEnabled ? '0 0 0 2px rgba(82,196,26,0.15)' : 'none' }}>Round-off{roundOffEnabled ? ' ✓' : ''}</Button>
