...
export default function InvoiceDetailPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            } catch { message.error('Failed to load Invoice'); } finally { setLoading(false); }
        };
        if (id) fetchData();
    }, [id]);
    ...
    const handleMarkPaid = async () => { if (!po) return; try { const updated = await purchasesApi.update(po.id, { invoice_status: 'complete' }); message.success('Invoice marked as paid'); setPo(updated); } catch { message.error('Failed to update'); } };
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            setPo(updated); setIsEditing(false); message.success('Invoice updated');
    ...
    const handleCopyToBuyer = () => {
        ...
        sessionStorage.setItem('inv_copy_items', JSON.stringify(copiedItems)); sessionStorage.setItem('inv_copy_source', po.po_number);
        setBuyerSelectModalOpen(false); navigate(`/app/purchases/invoice/create?buyerId=${selectedBuyerId}&buyerName=${encodeURIComponent(sel.name)}`);
    };
    ...
    if (!po) return <div className="text-center py-20"><Text type="secondary">Invoice not found</Text></div>;
        ...
        <div style={{ padding: '0 0 32px', maxWidth: '100%', overflowX: 'hidden' }} className="print-area">
            ...
            <div className="bg-white rounded-lg border border-gray-200 no-print" style={{ padding: '12px 24px', marginBottom: 20 }}>
                <div className="flex justify-between items-center">
                    <Space size="small"><Text strong style={{ fontSize: 14 }}>INVOICE DETAILS</Text><Tag color="green" style={{ borderRadius: 12 }}>Default Stock Store</Tag></Space>
                    {isEditing ? (
            ...
            <div className="bg-white rounded-lg border border-gray-200" style={{ padding: '32px 40px' }}>
                <Title level={3} style={{ textAlign: 'center', marginBottom: 28 }}>Invoice</Title>
                ...
                <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', marginBottom: 28 }}>
                    <div style={{ textAlign: 'center', background: '#f3f4f6', padding: 10, borderBottom: '1px solid #e5e7eb' }}><Text strong style={{ fontSize: 13 }}>Invoice Details</Text></div>
