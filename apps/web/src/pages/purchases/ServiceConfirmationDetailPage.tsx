...
export default function ServiceConfirmationDetailPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            } catch { message.error('Failed to load Service Confirmation'); } finally { setLoading(false); }
        };
        if (id) fetchData();
    }, [id]);
    ...
    const handleMarkComplete = async () => { if (!po) return; try { const updated = await purchasesApi.update(po.id, { invoice_status: 'complete' }); message.success('Service Confirmation marked as complete'); setPo(updated); } catch { message.error('Failed to update'); } };
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            setPo(updated); setIsEditing(false); message.success('Service Confirmation updated');
    ...
    const handleCopyToBuyer = () => {
        ...
        sessionStorage.setItem('sc_copy_items', JSON.stringify(copiedItems)); sessionStorage.setItem('sc_copy_source', po.po_number);
        setBuyerSelectModalOpen(false); navigate(`/app/purchases/service-confirmation/create?buyerId=${selectedBuyerId}&buyerName=${encodeURIComponent(sel.name)}`);
    };
    ...
    if (!po) return <div className="text-center py-20"><Text type="secondary">Service Confirmation not found</Text></div>;
        ...
        <div style={{ padding: '0 0 32px', maxWidth: '100%', overflowX: 'hidden' }} className="print-area">
            ...
            <div className="bg-white rounded-lg border border-gray-200 no-print" style={{ padding: '12px 24px', marginBottom: 20 }}>
                <div className="flex justify-between items-center">
                    <Space size="small"><Text strong style={{ fontSize: 14 }}>SERVICE CONFIRMATION DETAILS</Text><Tag color="green" style={{ borderRadius: 12 }}>Default Stock Store</Tag></Space>
                    {isEditing ? (
            ...
            <div className="bg-white rounded-lg border border-gray-200" style={{ padding: '32px 40px' }}>
                <Title level={3} style={{ textAlign: 'center', marginBottom: 28 }}>Service Confirmation</Title>
                ...
                <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', marginBottom: 28 }}>
                    <div style={{ textAlign: 'center', background: '#f3f4f6', padding: 10, borderBottom: '1px solid #e5e7eb' }}><Text strong style={{ fontSize: 13 }}>SC Details</Text></div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr' }}>
                        <div style={{ padding: '12px 16px', borderRight: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}><Text style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>SC Number</Text><Text strong style={{ fontSize: 13 }}>{po.po_number}</Text></div>
                        <div style={{ padding: '12px 16px', borderRight: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}><Text style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>SC Date</Text><Text strong style={{ fontSize: 13 }}>{dayjs(po.order_date).format('DD/MM/YYYY')}</Text></div>
                        <div style={{ padding: '12px 16px', borderRight: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}><Text style={{ fontSize: 11, color: '#9ca3af', display: 'block', marginBottom: 4 }}>Completion Date</Text>{isEditing ? <DatePicker size="small" value={editDeliveryDate} onChange={setEditDeliveryDate} format="DD/MM/YYYY" style={{ width: '100%' }} /> : <Text strong style={{ fontSize: 13 }}>{po.expected_delivery_date ? dayjs(po.expected_delivery_date).format('DD/MM/YYYY') : '-'}</Text>}</div>
