...
import SupplierDetailsDrawer from '@/features/purchases/components/drawers/SupplierDetailsDrawer';
...
export default function CreateServiceOrderPage() {
    ...
    const [items, setItems] = useState<SOLineItem[]>([{ key: 1, itemId: null, description: '', hsn: '', quantity: 0, units: '', currentStock: 0, price: 0, tax: 0, discount1: 0, discount2: 0 }]);
    const [supplier, setSupplier] = useSessionStorage<Party | null>('so_draft_supplier', null);
    ...
    const [placeOfSupplySource, setPlaceOfSupplySource] = useSessionStorage<'company' | 'supplier' | 'delivery'>('so_pos_source', 'delivery');
    
    const [buyerDetails, setBuyerDetails] = useLocalStorage<BuyerDetails>('so_buyer_details', {});
    const [userAddress, setUserAddress] = useLocalStorage<any>('so_user_address', { address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India' });
    const [deliveryLocation, setDeliveryLocation] = useLocalStorage<any>('so_delivery_location', { locationName: 'Main', address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India', billingLocations: [] });
    ...
    // Handle copied PO/SO items
    useEffect(() => {
        const storedItems = sessionStorage.getItem('so_copy_items');
        const sourcePo = sessionStorage.getItem('so_copy_source');
        if (storedItems) {
            try {
                ...
                if (Array.isArray(parsedItems) && parsedItems.length > 0) {
                    ...
                    message.success(`Items copied from ${sourcePo || 'previous Service Order'}`);
                    // Optionally set a note reference
                    form.setFieldsValue({ notes: `Ref: Copied from ${sourcePo}` });
                }
                sessionStorage.removeItem('so_copy_items');
                sessionStorage.removeItem('so_copy_source');
            } catch (e) { console.error('Error parsing copied items', e); }
        }
    }, [form]);

    useEffect(() => {
        if (supplierId && supplierId !== '(No ID)') {
            fetchPartyById(supplierId).then(data => { setSupplier((prev) => ({ ...data, billingLocations: (prev as Party & { billingLocations?: unknown[] })?.billingLocations || [] }) as Party); }).catch(() => { message.error('Failed to load supplier details'); });
        }
    }, [supplierId, form]);
    ...
    const handlePlaceOfSupplyClick = (source: 'company' | 'supplier' | 'delivery') => {
        ...
        else if (source === 'supplier') { city = supplier?.city || ''; state = supplier?.state || ''; country = supplier?.country || 'India'; }
    ...
    const handleSave = async (status: 'draft' | 'sent') => {
        try {
            ...
            const poNumber = values.so_number || `PO-${Date.now()}`;
            ...
            // Clear storage
            ['so_draft_supplier', 'so_pos_source'].forEach(k => sessionStorage.removeItem(k));
            ['so_buyer_details', 'so_user_address', 'so_delivery_location'].forEach(k => localStorage.removeItem(k));
            message.success(status === 'draft' ? 'Service Order saved as draft' : 'Service Order created successfully!');
            if (status === 'sent') {
                navigate(`/app/purchases/${result.id}?supplierName=${encodeURIComponent(supplier?.name || supplierNameParam || '')}`);
            } else {
    ...
    const handleDownloadTemplate = () => {
        const headers = ['Service ID', 'Service Description', 'HSN/SAC Code', 'Quantity', 'Units', 'Price', 'Tax %'];
        ...
        XLSX.writeFile(wb, 'service_item_table.xlsx');
    };

    const formatCurrency = (val: number) => `₹${val.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

    const itemDropdownOptions = [
        { value: -1, label: <span style={{ color: '#1677ff', fontWeight: 600 }}><PlusOutlined /> Add New Service</span> },
        ...inventoryItems.filter(inv => inv.product_service === 'service').map(inv => ({ value: inv.id, label: `${inv.sku} - ${inv.name}` }))
    ];
    ...
    const unitOptions = ['Kg', 'g', 'Liter', 'ml', 'Meter', 'cm', 'Piece', 'Box', 'Dozen', 'Ton', 'Bag', 'Roll', 'Hour', 'Day', 'Month'].map(u => ({ value: u, label: u }));

    const optionalColumnDefs: Record<string, any> = {
        alternateUnit: { title: 'Alt. Unit', dataIndex: 'alternateUnit', key: 'alternateUnit', width: 110, render: (_: any, record: SOLineItem) => <Select style={{ width: 100 }} placeholder="Unit" options={unitOptions} onChange={val => handleFieldChange(record.key, 'alternateUnit', val)} /> },
        discount1: { title: 'Discount 1 (%)', dataIndex: 'discount1', key: 'discount1', width: 130, render: (_: any, record: SOLineItem) => (
        ...
        discount2: { title: 'Discount 2 (%)', dataIndex: 'discount2', key: 'discount2', width: 130, render: (_: any, record: SOLineItem) => (
        ...
        amount: { title: 'Amount', dataIndex: 'amount', key: 'amount', width: 120, render: (_: any, record: SOLineItem) => {
            const net = getLineNetAmount(record);
            return <Text strong className="text-slate-700">{formatCurrency(net)}</Text>;
        }},
        totalTax: { title: 'Total Tax', dataIndex: 'totalTax', key: 'totalTax', width: 120, render: (_: any, record: SOLineItem) => {
            const net = getLineNetAmount(record);
    ...
    const baseColumns = [
        { title: 'Service ID', dataIndex: 'itemId', key: 'itemId', width: 200, render: (_: any, record: SOLineItem) => (
        ...
        { title: 'Service Description', dataIndex: 'description', key: 'description', render: (_: any, record: SOLineItem) => <Input value={record.description} onChange={e => handleFieldChange(record.key, 'description', e.target.value)} placeholder="Description" /> },
        { title: 'HSN/SAC Code', dataIndex: 'hsn', key: 'hsn', width: 120, render: (_: any, record: SOLineItem) => <Input style={{ width: 100 }} value={record.hsn} onChange={e => handleFieldChange(record.key, 'hsn', e.target.value)} /> },
        { title: 'Quantity', dataIndex: 'quantity', key: 'quantity', width: 90, render: (_: any, record: SOLineItem) => <Input type="number" value={record.quantity} onChange={e => handleFieldChange(record.key, 'quantity', Number(e.target.value))} style={{ width: 80 }} /> },
        { title: 'Units', dataIndex: 'units', key: 'units', width: 110, render: (_: any, record: SOLineItem) => <Select style={{ width: 100 }} value={record.units || undefined} onChange={val => handleFieldChange(record.key, 'units', val)} placeholder="Unit" options={unitOptions} /> },
        { title: 'Current Stock', dataIndex: 'currentStock', key: 'currentStock', width: 110, render: (_: any, record: SOLineItem) => <Text>{record.currentStock || '-'}</Text> },
        { title: priceType === 'regular' ? 'Price (Regular)' : priceType === 'wholesale' ? 'Price (Wholesale)' : 'Price (Default)', dataIndex: 'price', key: 'price', width: 130, render: (_: any, record: SOLineItem) => <Input type="number" value={record.price} onChange={e => handleFieldChange(record.key, 'price', Number(e.target.value))} style={{ width: 100 }} /> },
        { title: 'Tax', dataIndex: 'tax', key: 'tax', width: 110, render: (_: any, record: SOLineItem) => <Select style={{ width: 100 }} value={record.tax} onChange={val => handleFieldChange(record.key, 'tax', val)} options={taxOptions} /> },
    ];

    const itemColumns = [
        ...baseColumns,
        ...selectedOptionalColumns.map(key => optionalColumnDefs[key]).filter(Boolean),
        { title: '', key: 'action', width: 50, render: (_: any, record: SOLineItem) => <Button type="text" danger icon={<DeleteOutlined />} onClick={() => handleRemoveItem(record.key)} /> },
    ];
        ...
        <div style={{ maxWidth: '1536px', width: '100%', margin: '0 auto', paddingBottom: '40px' }}>
            ...
            <div className="bg-[#001529] text-white p-3 px-6 rounded-t-lg flex justify-between items-center mt-2 shadow-sm">
                <Space size="middle">
                    ...
                    <Title level={4} style={{ margin: 0, color: 'white' }}>Service Order</Title>
                </Space>
                ...
                <Row gutter={24}>
                    <Col span={8}>
                        <div className="flex flex-col gap-4 h-full">
                            ...
                            <Card size="small" className="border border-gray-200 shadow-sm rounded-xl overflow-hidden bg-white flex-1" title={<div className="text-slate-700 font-semibold text-[13px] uppercase tracking-wide">Supplier Details</div>} extra={<Button type="text" size="small" icon={<EditOutlined className="text-slate-400 hover:text-blue-600"/>} onClick={() => setIsSupplierDrawerOpen(true)} />} styles={{ header: { backgroundColor: '#fcfcfd', borderBottom: '1px solid #f1f5f9', padding: '12px 16px', minHeight: 'auto' }, body: { padding: '16px' } }}>
                                <div className="flex justify-between items-start mb-2"><Text strong className="text-[15px] text-slate-800">{supplier?.name || supplierNameParam || "Selected Supplier"}</Text><Text className="text-slate-700 font-medium text-[12px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded border border-blue-100">GSTIN: {supplier?.gstin || "26AALFP7255P1ZE"}</Text></div>
                                <Text className="text-slate-500 text-[12px] leading-relaxed block">{supplier?.address1 || "SURVEY NO.218/1/4, Unnamed Road"}<br/>{supplier?.city ? `${supplier.city}, ${supplier.state || ''}` : "Dadra & Nagar Haveli"}<br/>{supplier?.country || 'India'} - {supplier?.pincode || '396193'}</Text>
                                <div className="flex justify-end mt-4 pt-3 border-t border-slate-100"><Button type="link" size="small" className={`p-0 text-xs font-medium ${placeOfSupplySource === 'supplier' ? 'text-green-600 font-bold' : 'text-blue-600'}`} onClick={() => handlePlaceOfSupplyClick('supplier')}>{placeOfSupplySource === 'supplier' ? '✓ Place of Supply (Active)' : 'Place of Supply'}</Button></div>
                            </Card>
                        </div>
                    </Col>
                    <Col span={8}>
                        <div className="flex flex-col gap-4 h-full">
                            ...
                            <Card size="small" className="border border-gray-200 shadow-sm rounded-xl overflow-hidden bg-white flex-1" title={<div className="text-slate-700 font-semibold text-[13px] uppercase tracking-wide flex items-center gap-2">Place Of Supply <Tag color={placeOfSupplySource === 'company' ? 'blue' : placeOfSupplySource === 'supplier' ? 'purple' : 'green'} className="text-[10px] m-0 border-0">{placeOfSupplySource === 'company' ? 'From Company' : placeOfSupplySource === 'supplier' ? 'From Supplier' : 'From Delivery'}</Tag></div>} styles={{ header: { backgroundColor: '#fcfcfd', borderBottom: '1px solid #f1f5f9', padding: '12px 16px', minHeight: 'auto' }, body: { padding: '16px' } }}>
            ...
            <SupplierDetailsDrawer open={isSupplierDrawerOpen} onClose={() => setIsSupplierDrawerOpen(false)} supplier={supplier || {}} onSave={(data) => setSupplier(data as Party)} />
