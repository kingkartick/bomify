...
export default function CreatePurchaseOrderPage() {
    ...
    const [supplier, setSupplier] = useSessionStorage<Party | null>('po_draft_supplier', null);
    ...
    const [placeOfSupplySource, setPlaceOfSupplySource] = useSessionStorage<'company' | 'supplier' | 'delivery'>('po_pos_source', 'delivery');
    const [documentTabsData, setDocumentTabsData] = useState<DocumentTabsData>(EMPTY_TABS_DATA);
    
    const [buyerDetails, setBuyerDetails] = useLocalStorage<BuyerDetails>('po_buyer_details', {});
    const [userAddress, setUserAddress] = useLocalStorage<any>('po_user_address', { address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India' });
    const [deliveryLocation, setDeliveryLocation] = useLocalStorage<any>('po_delivery_location', { locationName: 'Main', address1: 'Main Address,', city: 'Mumbai', state: 'Maharashtra', pincode: '400001', country: 'India', billingLocations: [] });
    ...
    useEffect(() => {
        const storedItems = sessionStorage.getItem('po_copy_items');
        const sourcePo = sessionStorage.getItem('po_copy_source');
        if (storedItems) {
            try {
                ...
                sessionStorage.removeItem('po_copy_items');
                sessionStorage.removeItem('po_copy_source');
    ...
    const calculations = useMemo(() => {
        ...
        const baseTax = rcmEnabled ? 0 : validItems.reduce((sum, i) => sum + (getLineNetAmount(i) * (i.tax / 100)), 0);
        ...
        const totalTax = baseTax + extraChargesTax;
        const totalAfterTax = totalBeforeTax + totalTax + extraChargesTotal;
        ...
        return { totalBeforeTax, totalTax, totalAfterTax, roundOff, grandTotal, totalDiscount, extraChargesTotal };
    }, [items, rcmEnabled, roundOffEnabled, getLineNetAmount, documentTabsData.extra_charges]);
    ...
    const handleSave = async (status: 'draft' | 'sent') => {
        try {
            ...
            const poNumber = values.po_number || `PO-${Date.now()}`;
            ...
            // Clear storage
            ['po_draft_supplier', 'po_pos_source'].forEach(k => sessionStorage.removeItem(k));
            ['po_buyer_details', 'po_user_address', 'po_delivery_location'].forEach(k => localStorage.removeItem(k));
            message.success(status === 'draft' ? 'Purchase Order saved as draft' : 'Purchase Order created successfully!');
            if (status === 'sent') {
    ...
    const baseColumns = [
        ...
        { title: 'Current Stock', dataIndex: 'currentStock', key: 'currentStock', width: 130, render: (_: any, record: POLineItem) => {
            if (!record.inventoryItemData) return <Text>-</Text>;
            const stock = Number(record.currentStock || 0);
            const reorder = Number(record.inventoryItemData.reorder_level || 0);
            const isLow = stock <= reorder;
            const isMedium = !isLow && stock <= reorder * 2;
            return (
                <div>
                    <Text style={{ 
                        fontWeight: 600, 
                        color: isLow ? '#ff4d4f' : isMedium ? '#d48806' : '#52c41a',
                        fontSize: 13
                    }}>
                        {stock} {record.units || ''}
                    </Text>
                    {isLow && <div style={{ fontSize: 10, color: '#ff4d4f', marginTop: 2 }}>⚠ Low Stock</div>}
                </div>
            );
        }},
    ...
    return (
        <div style={{ maxWidth: '1536px', width: '100%', margin: '0 auto', paddingBottom: '40px' }}>
            {/* Tabs replaced with shared component below — no separate tabItems needed */}
            {/* Header */}
            <div className="bg-[#001529] text-white p-3 px-6 rounded-t-lg flex justify-between items-center mt-2 shadow-sm">
                <Space size="middle">
                    ...
                    <Title level={4} style={{ margin: 0, color: 'white' }}>Purchase Order</Title>
                </Space>
                ...
                <Row gutter={24}>
                    ...
                    <Col span={8}>
                        <Card size="small" className="border border-blue-100 shadow-[0_4px_12px_rgba(37,99,235,0.06)] rounded-xl bg-gradient-to-b from-white to-[#f8fafc] h-full overflow-hidden" title={<div className="text-blue-700 font-semibold text-[13px] uppercase tracking-wide">Primary Document Details</div>} extra={<Button type="text" size="small" className="text-xs text-slate-500 hover:text-slate-800 bg-white border border-slate-200 shadow-sm rounded-md px-2" onClick={() => setShowOptionalFields(!showOptionalFields)}>{showOptionalFields ? 'Hide Optional Fields' : 'Show Optional Fields'}</Button>} styles={{ header: { backgroundColor: '#f0f4f8', borderBottom: '1px solid #e2e8f0', padding: '12px 16px', minHeight: 'auto' }, body: { padding: '16px 20px' } }}>
                            <Form.Item label={<span className="text-xs font-medium text-slate-500 uppercase tracking-wide">Title <span className="text-red-500">*</span></span>} style={{ marginBottom: 20 }}><Input defaultValue={`Purchase Order dated ${dayjs().format('DD/MM/YYYY')}`} className="rounded-lg border-gray-300 py-1.5 font-medium text-slate-800" /></Form.Item>
                        ...
                        <Col span={10}>
                            <div className="p-4 rounded-md">
                                ...
                                {calculations.extraChargesTotal > 0 && (
                                    <div className="flex justify-between mb-2"><Text className="text-gray-500">Extra Charges :</Text><Text strong>{formatCurrency(calculations.extraChargesTotal)}</Text></div>
                                )}
