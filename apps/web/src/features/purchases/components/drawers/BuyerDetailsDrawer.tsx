...
interface BuyerDetailsDrawerProps {
  ...
  onClose: () => void;
  buyer: BuyerDetails;
  onSave: (buyer: BuyerDetails) => void;
}

export default function BuyerDetailsDrawer({ open, onClose, buyer, onSave }: BuyerDetailsDrawerProps) {
  const [form] = Form.useForm();
  const [billingLocations, setBillingLocations] = useState<any[]>(buyer.billingLocations || []);
  ...
  useEffect(() => {
    if (open) {
      ...
      setBillingLocations(buyer.billingLocations || []);
    }
  }, [open, buyer, form]);
  ...
  const persistLocations = (newLocations: any[]) => {
    ...
    onSave({ ...buyer, billingLocations: newLocations });
  };

  const handleSave = () => {
    form.validateFields().then(values => {
      onSave({ ...buyer, ...values, billingLocations });
      onClose();
    });
  };
  ...
  const handleSelectLocation = (loc: any) => {
    onSave({
      ...buyer,
      companyName: loc.locationName || loc.companyName || buyer.companyName,
      gstin: loc.gstin || buyer.gstin || '',
    ...
    message.success(`Location "${loc.locationName || 'Main'}" selected as Buyer address`);
    onClose();
  };
    ...
    <>
      ...
      >
        ...
        <div className="p-5 bg-white border-b border-gray-100 mb-2">
          <Text strong className="block mb-4 text-[#0fa387] text-sm uppercase tracking-wider">Edit Buyer Details</Text>
        ...
        <style>{`
          .buyer-details-modal .ant-modal-content { padding: 0; overflow: hidden; border-radius: 12px; box-shadow: 0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1); }
          .buyer-details-modal .ant-modal-header { background-color: #ffffff; border-bottom: 1px solid rgba(0,0,0,0.05); padding: 16px 20px; margin: 0; border-radius: 12px 12px 0 0; }
          .buyer-details-modal .ant-modal-title { color: #1f2937; font-weight: 500; }
          .buyer-details-modal .ant-form-item-label { padding-bottom: 4px; }
          .buyer-details-modal .ant-list-items > div:last-child { border-bottom: none !important; }
        `}</style>
      </Modal>
